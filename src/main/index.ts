...
import { applyLinuxImeWorkaround, detectRunningLinuxIm } from "./linux-ime.js";
...
applyLinuxImeWorkaround({
  ...
  runningIm: detectRunningLinuxIm(),
...
app.on("before-quit", (event) => {
  ...
  if (!sessionsClosed && controller) {
    ...
    if (!closingSessions) {
      ...
      void controller.closeAll().finally(() => { sessionsClosed = true; app.quit(); }).catch(() => {});
    }
    return;
  }
  tray?.destroy();
