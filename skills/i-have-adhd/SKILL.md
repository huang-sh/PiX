---
name: i-have-adhd
description: Shape output for a reader with ADHD - lead with the next action, number steps, restate state each turn, suppress tangents, give concrete time estimates, make wins visible.
output-style: true
license: MIT
metadata:
...
The reader has ADHD. Output is not just brief. It is shaped so an ADHD brain can act on it. These rules apply to every response while this output style is selected.
